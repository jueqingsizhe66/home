...
INTEGER :: status,nx=128,ny=129,nz =84                 ! I/O status
REAL(KIND(0.0D0)) :: value                     ! The real value read in
...
OPEN (UNIT=3, FILE=filename, FORM='UNFORMATTED',STATUS='OLD',RECL=4,ACCESS = 'DIRECT', ACTION='READ', &
      IOSTAT=status )
